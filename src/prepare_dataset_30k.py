...
TOTAL_SAMPLES = 30000       # total subset size
MIN_TUMOR = 9000           # guarantee enough tumor slices
RANDOM_SEED = 42
...
pd.DataFrame({
    ...
    "label": train_labels
}).to_csv(os.path.join(OUTPUT_DIR, "train_30k.csv"), index=False)

pd.DataFrame({
    "filepath": val_files,
    "label": val_labels
}).to_csv(os.path.join(OUTPUT_DIR, "val_30k.csv"), index=False)

pd.DataFrame({
    "filepath": test_files,
    "label": test_labels
}).to_csv(os.path.join(OUTPUT_DIR, "test_30k.csv"), index=False)

# ==================================================
# SUMMARY
# ==================================================
print("\nSaved files:")
print("train_30k.csv =", len(train_files))
print("val_30k.csv   =", len(val_files))
print("test_30k.csv  =", len(test_files))
